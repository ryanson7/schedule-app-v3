...
// 🔥 공통 상태 처리 함수 (approval_status 기반)
const getStatusInfo = (approvalStatus: string, isActive: boolean) => {
  if (isActive === false) {
    if (approvalStatus === "cancelled") {
      return { text: "취소완료", color: "#dc2626", bgColor: "#fef2f2", borderColor: "#dc2626" };
    } else if (approvalStatus === "deleted") {
      return { text: "삭제완료", color: "#dc2626", bgColor: "#fef2f2", borderColor: "#dc2626" };
    }
  }

  switch (approvalStatus) {
    case "pending":
      return { text: "임시저장", color: "#6b7280", bgColor: "#f9fafb", borderColor: "#6b7280" };
    case "approval_requested":
      return { text: "승인요청", color: "#2563eb", bgColor: "#eff6ff", borderColor: "#2563eb" };
    case "approved":
      return { text: "승인완료", color: "#059669", bgColor: "#f0fdf4", borderColor: "#059669" };
    case "confirmed":
      return { text: "확정완료", color: "#059669", bgColor: "#f0fdf4", borderColor: "#059669" };
    case "modification_requested":
      return { text: "수정요청", color: "#8b5cf6", bgColor: "#faf5ff", borderColor: "#8b5cf6" };
    case "modification_approved":
      return { text: "수정중", color: "#f59e0b", bgColor: "#fffbeb", borderColor: "#f59e0b" };
    case "cancellation_requested":
      return { text: "취소요청", color: "#f59e0b", bgColor: "#fffbeb", borderColor: "#f59e0b" };
    case "deletion_requested":
      return { text: "삭제요청", color: "#dc2626", bgColor: "#fef2f2", borderColor: "#dc2626" };
    default:
      return { text: "상태 미정", color: "#6b7280", bgColor: "#f9fafb", borderColor: "#6b7280" };
  }
};

// ✅ 학원 전용: tracking_status 기반 상태(크로스체크) 우선
const getAcademyTrackingStatusInfo = (trackingStatus?: string | null) => {
  if (trackingStatus === "crosscheck_req") {
    return { text: "크로스체크 요청", color: "#0ea5e9", bgColor: "#f0f9ff", borderColor: "#0ea5e9" };
  }
  if (trackingStatus === "crosscheck_ok") {
    return { text: "크로스체크 완료", color: "#0284c7", bgColor: "#e0f2fe", borderColor: "#0284c7" };
  }
  return null;
};
...
export const UnifiedScheduleCard = ({
  ...
  // ✅ 학원은 tracking_status 우선, 스튜디오는 무시
  const academyTracking = scheduleType === "academy" ? getAcademyTrackingStatusInfo(schedule?.tracking_status) : null;

  const statusInfo = academyTracking || getStatusInfo(safeSchedule.approvalStatus, schedule?.is_active);
    ...
    >
      ...
      {showShooterInfo ? (
        <>
          ...
          >
            <div style={{ flex: 1, wordWrap: "break-word", wordBreak: "break-word", whiteSpace: "normal" }}>
        ...
        <>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1 }}>
              ...
              <span style={{ color: "#9ca3af" }}>|</span>
              <span style={{ wordWrap: "break-word", wordBreak: "break-word" }}>{safeSchedule.professorName}</span>
            </div>
            ...
            {scheduleType === "academy" && showCheckbox && (
              <input
                ...
                onClick={(e) => e.stopPropagation()}   // 기존 유지
                style={{
                  transform: "scale(1.1)",
                  cursor: "pointer",
                  accentColor: "#2563eb",
